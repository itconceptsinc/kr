...
from analysis_dashboard.analysis_callbacks import update_circuit_anomalies_table_callback,\
    ...
    update_gtfs_hist_callback, update_gtfs_preds_callback, line_colors, stops, heading

line_direction = {'East/Northbound':1,'West/SouthBound':2}
                                ...
                                html.Div(
                                    ...
                                    children=[
                                        dcc.Dropdown(
                                            ...
                                            options=[
                                                {"label": i, "value": i}
                                                for i in line_direction
                                            ],
                                            value='East/Northbound',
...
@app.callback(Output('gtfs-preds', 'children'),
              [Input('gtfs-preds-update', 'n_intervals'),
               Input('heading-dropdown','value'),
               Input('stop-dropdown','value')])
def update_gtfs_preds(n_interval,heading,stop):
    return update_gtfs_preds_callback(heading,stop)
